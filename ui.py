...
# import partial to resolve the bug of deleting the last task when delete is clicked
...
def show_add_task_frame(app):
    ...
    entry = tk.Entry(master=frame)
    entry.grid()
    # to show the task after adding the task and clicking submit to show the original list with new task
    btn = tk.Button(master=frame, text="Submit", command = lambda: submit_task(entry.get(), app))
    btn.grid()

    # bring the frame into view to cover the previous one
    frame.tkraise()

# function to show frame showing all tasks 
def show_all_tasks_frame(app):
    ...
    # show the frame with the above tasks/makes it alive or active
    frame.tkraise()
